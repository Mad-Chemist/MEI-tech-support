...
class MeiadminFileMailCoordinator
{
    ...
    protected function _loadCustomers()
    {
        ...
        $query->select('u.email')
            ...
            ->leftJoin('#__meiadmin_customer_subscriptions as s ON (s.fk_user_id = u.id)')
            ->leftJoin('#__meiadmin_customer_exclusions as e ON (e.fk_user_id = s.fk_user_id)')
            ->where('s.fk_product_id ='.$this->_db->quote($this->_productId).' AND e.file_type <> '.$this->_db->quote($this->_type))
            ->group('u.email');
        $this->_db->setQuery($query);
