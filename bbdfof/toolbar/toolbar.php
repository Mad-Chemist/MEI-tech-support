...
class BBDFOFToolbar
{
	...
	protected function renderSubmenu()
	{
		...
		foreach ($views as $view)
		{
			...
			{
				$altview = BBDFOFInflector::isPlural($view) ? BBDFOFInflector::singularize($view) : BBDFOFInflector::pluralize($view);
	...
	protected function getMyViews()
	{
		...
		if (!empty($allFolders))
			foreach ($allFolders as $folder)
			{
				...
				// View already added
				if (in_array(BBDFOFInflector::pluralize($view), $t_views))
					continue;
				...
				if (!$meta)
				{
					$plural = BBDFOFInflector::pluralize($view);
				...
				if (!empty($meta))
				{
					...
					$order = (int) $xml->BBDFOFlib->ordering;
				}
				else
				{
