...
// require this. Why? No idea whatsoever. If I remove it, BBDFOF crashes on some
// hosts. Same PHP version on another host and no problem occurs. Any takers?
if (class_exists('BBDFOFTable', false))
{
	return;
}
 ...
 */
class BBDFOFTable extends JObject
{
	/**
	 * Include paths for searching for BBDFOFTable classes.
	 *
	 * @var    array
	 */
	...
	public static function &getAnInstance($type = null, $prefix = 'JTable', $config = array())
	{
		...
		if (!array_key_exists($tableClass, $instances))
		{
			...
			if (!class_exists($tableClass))
			{
				$tableClass = 'BBDFOFTable';
			}

			$tbl_common = str_replace('com_', '', $config['option']) . '_';
			if (!array_key_exists('tbl', $config))
			{
				$config['tbl'] = strtolower('#__' . $tbl_common . strtolower(BBDFOFInflector::pluralize($type)));
			}
			if (!array_key_exists('tbl_key', $config))
			{
				$keyName = BBDFOFInflector::singularize($type);
	 ...
	 * to the BBDFOFTable instance properties.
	 *
	...
	/**
	 * Method to bind an associative array or object to the BBDFOFTable instance.This
	 * method only binds properties that are publicly accessible and optionally
	 * takes an array of properties to ignore when binding.
	 *
	 * @param   mixed  $src     An associative array or object to bind to the BBDFOFTable instance.
	...
	/**
	 * Method to store a row in the database from the BBDFOFTable instance properties.
	 * If a primary key value is set the row with that primary key value will be
	 * updated with the instance property values.  If no primary key value is set
	 * a new row will be inserted into the database with the properties from the
	 * BBDFOFTable instance.
	 *
	...
	function isCheckedOut($with = 0, $against = null)
	{
		...
		if (isset($this) && is_a($this, 'BBDFOFTable') && is_null($against))
		{
	...
	public function delete($oid = null)
	{
		...
		if ($this->_trackAssets)
		{
			...
			//$asset = self::getAnInstance('Asset');

			$asset = Jtable::getInstance('Asset', 'JTable', array('dbo' => $this->getDbo())); // Replaced by Nik
	...
	protected function onBeforeBind(&$from)
	{
		...
		{
			$name = BBDFOFInflector::pluralize($this->getKeyName());
	...
	protected function onBeforeStore($updateNulls)
	{
		...
		if ($hasTitle && $hasSlug)
		{
			if (empty($this->$slug))
			{
				...
				$this->$slug = BBDFOFStringUtils::toSlug($this->$title);
			}
			else
			{
				// Filter the slug for invalid characters
				$this->$slug = BBDFOFStringUtils::toSlug($this->$slug);
			}
	...
	public function setInput(BBDFOFInput $input)
	{
		$this->input = $input;
	}
	...
	/**
	 * Add a filesystem path where BBDFOFTable should search for table class files.
	 * You may either pass a string or an array of paths.
	 *
	 * @param   mixed  $path  A filesystem path or array of filesystem paths to add.
	 *
	 * @return  array  An array of filesystem paths to find BBDFOFTable classes in.
	 */
	 ...
	 *
	 * @param   BBDFOFTable  $table  A BBDFOFTable object for the asset parent.
	...
	/**
	 * Method to provide a shortcut to binding, checking and storing a BBDFOFTable
	 * instance to the database table.  The method will check a row in once the
	 * data has been stored and if an ordering filter is present will attempt to
	 * reorder the table rows based on the filter.  The ordering filter is an instance
	 * property name.  The rows that will be reordered are those whose value matches
	 * the BBDFOFTable instance for the property specified.
	 *
	 * @param   mixed   $src             An associative array or object to bind to the BBDFOFTable instance.
